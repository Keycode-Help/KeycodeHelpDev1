...
export default function AuthForm({ mode, onSubmit, initial = {} }) {
  ...
  const renderFields = () => {
    ...
    fields.push(
      <FormField
        ...
        required
        autoComplete={mode === 'login' || mode === 'admin-login' ? 'username' : 'email'}
      />
    );
  ...
  const getSubmitButtonText = () => {
    switch (mode) {
      ...
      case 'admin-login': return 'Access Admin Panel';
      case 'admin-register': return 'Create Admin Account';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button
        type="submit"
        className="w-full rounded-3xl font-bold border-0 bg-white text-black hover:bg-yellow-500 hover:text-black transition duration-100 focus:outline-none focus:ring-0 py-3"
      >
        {getSubmitButtonText()}
      </button>
    </form>
