...
const MakeDropDown = ({ selectedMake, options, onChange }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-[9999] w-full mt-2 bg-slate-900/95 backdrop-blur-sm border border-slate-600 rounded-xl shadow-2xl max-h-80 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent">
          <div className="p-2">
            {options.length === 0 ? (
              ...
              <>
                <div className="px-3 py-2 text-slate-500 text-xs border-b border-slate-600 mb-2">
                ...
                {options.map((make) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-3 py-2 text-left text-white hover:bg-slate-700 rounded-lg transition-colors duration-200 focus:outline-none focus:bg-slate-700"
                  >
                    <img
