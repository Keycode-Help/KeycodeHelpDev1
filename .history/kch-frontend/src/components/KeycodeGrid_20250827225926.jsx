...
const KeycodeGrid = ({ portals }) => {
  ...
  const handleCopyCredentials = async (portal) => {
    ...
    try {
      const response = await fetch('/api/keycodes/creds', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ oem: portal.id }),
      });
      ...
      if (response.ok) {
        ...
        setCopyStatus(prev => ({ 
          ...prev, 
          [portal.id]: errorData.error || 'Failed to copy credentials' 
        }));
      }
    } catch (error) {
      ...
      setCopyStatus(prev => ({ 
        ...prev, 
        [portal.id]: 'Failed to copy credentials' 
      }));
    } finally {
            ...
            <div className="mb-4 p-3 bg-gray-50 rounded-md">
              ...
              <div className="space-y-1">
                <div className="text-xs font-mono text-gray-600">
                  {portal.auth.username_env}
                </div>
                <div className="text-xs font-mono text-gray-600">
                  {portal.auth.password_env}
                </div>
              </div>
            </div>
