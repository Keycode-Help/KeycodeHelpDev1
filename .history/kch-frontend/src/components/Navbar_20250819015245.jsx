    ...
    <>
      ...
      {isBaseUser(userRole) && (
        <>
          ...
          <Link className="nav-link" to="/vehicle-keycode-request" onClick={() => setOpen(false)}>
            Request Keycode
          </Link>
          <Link className="nav-link" to="/subscriptions" onClick={() => setOpen(false)}>
            Subscriptions
          </Link>
          <Link className="nav-link" to="/user-dash" onClick={() => setOpen(false)}>
            User Dashboard
          </Link>
          <Link className="nav-link" to="/user-profile" onClick={() => setOpen(false)}>
            Profile
          </Link>
      ...
      {canSeeAdmin(userRole) && !isSuper(userRole) && (
        <>
          ...
          <Link className="nav-link" to="/admin/registered-users" onClick={() => setOpen(false)}>
            Registered Users
          </Link>
          <Link className="nav-link" to="/admin/document-validation" onClick={() => setOpen(false)}>
            Document Validation
          </Link>
          <Link className="nav-link" to="/admin/user-history" onClick={() => setOpen(false)}>
            User History
          </Link>
        </>
      )}
      ...
      {isSuper(userRole) && (
        <>
          ...
          <Link className="nav-link" to="/super-admin" onClick={() => setOpen(false)}>
      ...
      {userRole && (
        <button className="rounded-full bg-red-500/90 hover:bg-red-500 text-white px-4 py-2 font-medium" onClick={onClickLogout}>
          Logout
        </button>
      )}
    </>
