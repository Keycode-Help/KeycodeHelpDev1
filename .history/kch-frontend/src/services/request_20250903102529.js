...
instance.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Handle 403 errors - these usually indicate JWT secret mismatch
    if (status === 403) {
      const isAuthMeCall = url.includes("/auth/me");
      if (isAuthMeCall) {
        // For /auth/me calls, 403 usually means JWT secret mismatch - clear auth state
        console.warn("403 on /auth/me - JWT token invalid, clearing auth state");
        localStorage.removeItem("auth_user");
        localStorage.removeItem("auth_token");
        // Clear cookies
        document.cookie =
