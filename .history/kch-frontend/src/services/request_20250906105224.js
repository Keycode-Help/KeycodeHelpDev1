    ...
    try {
      ...
      if (session?.access_token) {
        ...
        if (import.meta.env.DEV) {
          console.log("🔑 Using Supabase session token:", `Bearer ${token.substring(0, 20)}...`);
        }
      } else {
        // Fallback to cookies or localStorage
        token = getCookie("access_token") || getTokenFromStorage();
        if (token && import.meta.env.DEV) {
          console.log("🔑 Using fallback token:", `Bearer ${token.substring(0, 20)}...`);
        }
      }
...
instance.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Check if we have a Supabase session for refresh
    let hasRefreshToken = false;
    try {
      const { data: { session } } = await supabase.auth.getSession();
      hasRefreshToken = !!session?.refresh_token;
    } catch (error) {
      console.warn("Failed to check Supabase session for refresh:", error);
    }
    ...
    if (
      ...
      try {
        console.log(
          `🔄 Attempting Supabase token refresh for ${status} error on ${url}`
        );
        ...
        if (data.session) {
          console.log("✅ Supabase token refresh successful, retrying original request");
