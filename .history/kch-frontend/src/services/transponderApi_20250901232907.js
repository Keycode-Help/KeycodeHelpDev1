...
class TransponderAPI {
   ...
   */
  static async testDatabaseContents() {
    try {
      console.log("🧪 Testing database contents...");

      // Check transponder_data table
      console.log("🔍 Checking transponder_data table...");
      const { count: dataCount, error: dataCountError } = await supabase
        .from("transponder_data")
        .select("*", { count: "exact", head: true });

      if (dataCountError) {
        console.error("❌ Data table count error:", dataCountError);
      } else {
        console.log("📊 Total rows in transponder_data:", dataCount);
      }

      // Get sample data from transponder_data
      ...
      console.log("📋 Sample data from transponder_data:", data);
      console.log("🔢 Actual data count:", data?.length || 0);

      // Check column structure
      if (data && data.length > 0) {
        console.log("🏗️ Column structure:", Object.keys(data[0]));
      }
    } catch (error) {
  ...
  /**
   * Check table structures to understand column mapping
   */
  static async checkTableStructures() {
    try {
      console.log("🔍 Checking table structures...");

      // Check transponder_data structure
      ...
      if (dataError) {
        console.error("❌ Error getting transponder_data sample:", dataError);
        return;
      }

      if (dataSample && dataSample.length > 0) {
        console.log("📋 transponder_data columns:", Object.keys(dataSample[0]));
        console.log("📋 transponder_data sample:", dataSample[0]);

        // Show key vehicle information
        ...
        console.log("🚗 Vehicle Info:", {
          ...
          year_display: sample.year_display,
          system_type: sample.system_type,
          transponder_family: sample.transponder_family,
        });
      }
    } catch (error) {
      console.error("❌ Check table structures error:", error);
    }
  }
}
