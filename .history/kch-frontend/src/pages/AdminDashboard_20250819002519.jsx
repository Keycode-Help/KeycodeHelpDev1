      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <section className="rounded-3xl border border-neutral-800 bg-black/30 p-5 backdrop-blur supports-[backdrop-filter]:bg-black/40">
            <h2 className="text-white font-semibold text-lg mb-3">Subscriptions</h2>
                  ...
                  <tbody>
                    {subscriptions.map((s) => (
                      <tr key={s.id} className="border-t border-neutral-800">
                        ...
                        <td className="py-2">{s.userEmail || "User Inactive"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <section className="lg:col-span-2 rounded-3xl border border-neutral-800 bg-black/30 p-5 backdrop-blur supports-[backdrop-filter]:bg-black/40">
            <h2 className="text-white font-semibold text-lg mb-3">Pending Requests</h2>
            {pendingRequests.length === 0 ? (
              <p className="text-gray-400">No pending requests at the moment.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {pendingRequests.map((request) => (
                  <div className="rounded-2xl border border-neutral-800 bg-black/40 p-4 hover:border-neutral-700 transition-colors" key={request.id}>
                    ...
                    <p className="text-gray-300">VIN: <span className="text-white font-medium">{request.vin}</span></p>
                    <p className="text-gray-300">Price: <span className="text-white font-medium">${request.price}</span></p>
                    <p className="text-gray-300">Email: <span className="text-white font-medium">{request.keycodeUserEmail || "N/A"}</span> {" "}
                      <span className={request.isValidatedUser ? "text-emerald-400" : "text-red-400"}>
                        {request.isValidatedUser ? "Validated" : "Unvalidated"}
                      </span>
                    </p>
                    <div className="flex flex-wrap gap-2 mt-3">
                      <img src={request.frontId} alt="Front ID" className="w-24 h-24 object-cover rounded-lg border border-neutral-700" onClick={() => openModal(request.frontId)} />
                      <img src={request.backId} alt="Back ID" className="w-24 h-24 object-cover rounded-lg border border-neutral-700" onClick={() => openModal(request.backId)} />
                      <img src={request.registration} alt="Registration" className="w-24 h-24 object-cover rounded-lg border border-neutral-700" onClick={() => openModal(request.registration)} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Transactions */}
          <section className="rounded-3xl border border-neutral-800 bg-black/30 p-5 backdrop-blur supports-[backdrop-filter]:bg-black/40">
            <h2 className="text-white font-semibold text-lg mb-3">Transactions</h2>
                  ...
                  <tbody>
                    {transactions.map((t) => (
                      <tr key={t.id} className="border-t border-neutral-800 align-top">
                        ...
                        <td className="py-2">
                          {t.vehicles.map((v) => (
                            <div key={v.id}>{v.make?.manufacturerName || "Unknown Make"} {v.model} (VIN: {v.vin})</div>
                          ))}
                        </td>
                        <td className="py-2">
                          {t.vehicles.map((v) => (
                            <div key={v.id}>{v.keycode ? v.keycode : "Not Provided"}</div>
                          ))}
                        </td>
                      </tr>
                    ))}
          ...
          {/* In Progress */}
          <section className="lg:col-span-2 rounded-3xl border border-neutral-800 bg-black/30 p-5 backdrop-blur supports-[backdrop-filter]:bg-black/40">
            <h2 className="text-white font-semibold text-lg mb-3">In Progress Requests</h2>
            {inProgressRequests.length === 0 ? (
              <p className="text-gray-400">No In Progress Requests at the moment.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  ...
                  <div className="rounded-2xl border border-neutral-800 bg-black/40 p-4 hover:border-neutral-700 transition-colors" key={request.id}>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <input
                        ...
                        onChange={(e) => handleKeycodeChange(request.id, e.target.value)}
                      ...
                      <input
                        ...
                        onChange={(e) => handlePinChange(request.id, e.target.value)}
