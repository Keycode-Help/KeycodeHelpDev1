  ...
  return (
    <div className="mx-auto max-w-5xl px-4 md:px-6 py-8">
      ...
      <div className="rounded-3xl border border-neutral-800 bg-black/30 p-5 backdrop-blur supports-[backdrop-filter]:bg-black/40">
        ...
        <div className="mt-6">
          <h2 className="text-white font-semibold mb-2">General eligibility & document requirements</h2>
          <ul className="list-disc pl-5 text-gray-200 space-y-1">
            <li>Service is available only within the United States and Canada.</li>
            <li>All documents must be valid, non-expired, and clearly legible in the photo.</li>
            <li>Driver’s license (owner) is required.</li>
            <li>Provide one of the following: registration, title, or proof of insurance.</li>
            <li>Upload a photo of the vehicle’s license plate and a photo of the odometer.</li>
          </ul>
        </div>

        <div className="mt-6">
          <h2 className="text-white font-semibold mb-2">States requiring a locksmith license</h2>
          <p className="text-gray-300 text-sm mb-3">If you operate in one of the states below, a locksmith license is required to perform locksmith-related services. Always verify current requirements with the state regulator.</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
            {LICENSED_STATES.map((s) => (
              <div key={s} className="rounded-xl border border-neutral-800 bg-black/40 px-3 py-2 text-gray-200 text-sm">
                {s}
              </div>
            ))}
          </div>
