...
function UserManagement() {
  ...
  const UserCard = ({ user }) => {
    ...
    return (
      <div className={`bg-slate-800/50 rounded-2xl p-6 border transition-all duration-300 shadow-lg hover:shadow-xl ${
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-medium ${
                  ...
                  urgency === "urgent" ? "text-orange-300" :
                  urgency === "warning" ? "text-yellow-300" : "text-blue-300"
                }`}>
  ...
  const criticalTrials = trialStats.filter((u) => getTrialUrgency(u) === "critical").length;
  const urgentTrials = trialStats.filter((u) => getTrialUrgency(u) === "urgent").length;
  const warningTrials = trialStats.filter((u) => getTrialUrgency(u) === "warning").length;
                    ...
                    <div className={`p-4 rounded-lg border ${
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Timer className={`w-5 h-5 ${
                            getTrialUrgency(selectedUser) === "critical" ? "text-red-400" : 
                            getTrialUrgency(selectedUser) === "urgent" ? "text-orange-400" :
                            getTrialUrgency(selectedUser) === "warning" ? "text-yellow-400" : "text-blue-400"
                          }`} />
                          <span className={`font-medium ${
                            getTrialUrgency(selectedUser) === "critical" ? "text-red-300" : 
                            getTrialUrgency(selectedUser) === "urgent" ? "text-orange-300" :
                            getTrialUrgency(selectedUser) === "warning" ? "text-yellow-300" : "text-blue-300"
                          }`}>
                            Time Remaining
                          </span>
                        </div>
                      ...
                      {selectedUser.trialEndsAt && (
                        <div className="mt-2 text-sm text-white/70">
                          Expires: {new Date(selectedUser.trialEndsAt).toLocaleString()}
                        </div>
                      )}
                    </div>
                  </div>
