...
function NastfCompliance() {
  ...
  const fetchComplianceData = async () => {
    ...
    try {
      ...
      const response = await api.get(`/admin/nastf-compliance${filter !== 'all' ? `?status=${filter}` : ''}`);
      setComplianceData(response.data);
      console.log("✅ NASTF compliance data fetched:", response.data);
    } catch (error) {
      console.error("❌ Error fetching compliance data:", error);
      setError(`Failed to fetch compliance data: ${error.response?.data || error.message}`);
  ...
  const downloadDocuments = async (vehicleId, vin) => {
    ...
    try {
      ...
      const response = await api.get(`/admin/nastf-compliance/${vehicleId}/documents`);
      ...
      // Download JSON file
      const jsonBlob = new Blob([JSON.stringify(d1FormData, null, 2)], { type: 'application/json' });
      ...
      if (documents.frontId) {
        const frontBlob = new Blob([Uint8Array.from(atob(documents.frontId), c => c.charCodeAt(0))], { type: 'image/jpeg' });
      ...
      if (documents.backId) {
        const backBlob = new Blob([Uint8Array.from(atob(documents.backId), c => c.charCodeAt(0))], { type: 'image/jpeg' });
      ...
      if (documents.registration) {
        const regBlob = new Blob([Uint8Array.from(atob(documents.registration), c => c.charCodeAt(0))], { type: 'image/jpeg' });
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-8 text-center">
            ...
            <p className="text-red-300">
              {error || "You don't have permission to access NASTF compliance data."}
            </p>
          </div>
        </div>
      </div>
  ...
  const getUrgencyColor = (item) => {
    ...
    if (item.isUrgent) return "text-orange-400 bg-orange-500/10 border-orange-500/30";
    if (item.daysRemaining <= 1) return "text-yellow-400 bg-yellow-500/10 border-yellow-500/30";
      ...
      <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`px-3 py-1 rounded-full text-xs font-bold ${getUrgencyColor(item)}`}>
                        {getUrgencyText(item)}
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <div className="text-white">{item.year} {item.make} {item.model}</div>
                      </div>
                      <div>
                        <div className="text-white/60 mb-1">Customer</div>
                        <div className="text-white">{item.userName}</div>
                        <div className="text-white/70 text-xs">{item.userEmail}</div>
                      </div>
                      <div>
                  ...
                  <div className="lg:w-64">
                    <div className="text-white/60 text-xs mb-2">Available Documents:</div>
                    <div className="flex gap-2 mb-3">
                      <span className={`text-xs px-2 py-1 rounded ${item.hasFrontId ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                        Front ID
                      </span>
                      <span className={`text-xs px-2 py-1 rounded ${item.hasBackId ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                        Back ID
                      </span>
                      <span className={`text-xs px-2 py-1 rounded ${item.hasRegistration ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                        Registration
                      </span>
                    </div>

                    <button
                      onClick={() => downloadDocuments(item.vehicleId, item.vin)}
        ...
        <div className="mt-8 p-6 bg-slate-800/50 border border-slate-600 rounded-xl">
          ...
          <div className="grid md:grid-cols-2 gap-6 text-sm">
            <div>
              <h4 className="text-white font-medium mb-2">Compliance Timeline:</h4>
              <ul className="space-y-1 text-white/70">
                <li>• D1 forms must be completed within 4 days of keycode order</li>
