...
function AdminDashboard() {
  ...
  const [trialActivating, setTrialActivating] = useState(new Set()); // Track which users are having trials activated
  ...
  const getImageUrl = useCallback((imageData) => {
    if (!imageData) {
      console.log("🔍 getImageUrl: No image data provided");
      return null;
    }

    console.log("🔍 getImageUrl: Processing image data", {
    ...
    console.log("🔍 getImageUrl: Generated URL", {
      url: optimizedUrl ? optimizedUrl.substring(0, 50) + "..." : "null",
      isBlob: optimizedUrl ? optimizedUrl.startsWith("blob:") : false,
      isData: optimizedUrl ? optimizedUrl.startsWith("data:") : false,
    });
  ...
  // Activate 3-day trial for a user
  const handleActivateTrial = async (userId) => {
    setTrialActivating(prev => new Set(prev).add(userId));
    
    try {
      const response = await api.post(`/admin/users/${userId}/activate-trial`, {
        trialDays: 3,
        activatedBy: user?.email
      });
      
      if (response.status === 200) {
        // Update the user in pendingUsers to show trial is active
        setPendingUsers(prev => 
          prev.map(u => 
            u.id === userId 
              ? { ...u, trialActive: true, trialEndsAt: response.data.trialEndsAt }
              : u
          )
        );
      ...
      console.error(`❌ Failed to activate trial for user ${userId}:`, error);
      handleApiError(
        error,
        `Failed to activate trial for user. ${error.response?.data?.message || 'Please try again.'}`
      );
    } finally {
      setTrialActivating(prev => {
                      ...
                      <div className="flex flex-col sm:flex-row lg:flex-col gap-3 w-full sm:w-auto lg:w-full">
                        ...
                        {!user.trialActive && (
                          <button
                            ...
                            disabled={trialActivating.has(user.id)}
                            className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-6 py-3 rounded-xl font-semibold hover:from-blue-400 hover:to-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-900 transition-all duration-300 shadow-lg hover:shadow-xl flex items-center justify-center gap-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {trialActivating.has(user.id) ? (
                              <>
                        ...
                        {user.trialActive && (
                          <div className="bg-gradient-to-r from-green-500/20 to-green-600/20 border border-green-500/30 text-green-300 px-6 py-3 rounded-xl font-semibold flex items-center justify-center gap-2 text-sm">
