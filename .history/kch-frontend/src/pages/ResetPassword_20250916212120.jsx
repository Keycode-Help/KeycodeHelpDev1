    ...
    try {
      ...
      if (response.ok) {
        ...
        if (errorMessage.includes("Invalid or expired reset token")) {
          errorMessage = "❌ This reset link has expired or has already been used. Please request a new password reset.";
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          ...
          <div className="text-center space-y-2">
            <button
              type="button"
              onClick={() => navigate("/login")}
              className="text-sm text-blue-400 hover:text-blue-300 underline transition-colors duration-200 block w-full"
            >
              Back to Login
            </button>
            
            {message && message.includes("expired or has already been used") && (
              <button
                type="button"
                onClick={() => navigate("/login?tab=reset")}
                className="text-sm text-green-400 hover:text-green-300 underline transition-colors duration-200 block w-full"
              >
