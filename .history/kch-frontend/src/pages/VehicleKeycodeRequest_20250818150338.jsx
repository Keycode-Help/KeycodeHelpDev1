...
function VehicleKeycodeRequest() {
  ...
  const handleFileChange = (e) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setErrors((prev) => ({ ...prev, [name]: "File size should not exceed 5MB." }));
      return;
    }
  ...
  const handleSubmit = (e) => {
    ...
    axios
      ...
      .catch((error) => {
        ...
        alert(error.response?.data || "Failed to request keycode. Please try again.");
      });
  };
        ...
        <form onSubmit={handleSubmit} className="keycode-form">
          ...
          {selectedMake && (
            <div className="pricing-info">
              <h3 style={{ color: "#00ff85", marginBottom: "15px" }}>Keycode Pricing</h3>

              <div className="price-row">
                <span className="price-label">Non-Member Price:</span>
                <span className="price-value">${selectedMakePrice.toFixed(2)}</span>
              </div>

              <div className="price-row">
                <span className="price-label">Member Price:</span>
                <span className="price-value member-price">
                  {selectedMemberPrice > 0 ? `$${selectedMemberPrice.toFixed(2)}` : "Ask for pricing"}
                </span>
              </div>
              ...
              {requiresPinCode && (
                <div className="pin-notice">
                  <span style={{ color: "#f59e0b", fontWeight: "bold" }}>⚠️ PIN Code Required</span>
                  <p style={{ color: "#f59e0b", fontSize: "0.9rem", margin: "5px 0" }}>
              ...
              {!isAuthenticated && (
                <div className="member-benefit">
                  <p style={{ color: "#10b981", fontSize: "0.9rem", margin: "10px 0" }}>
          ...
          <div className="form-group">
            ...
            <input
              ...
              className="form-control"
            />
            {errors.frontId && <p className="error-message">{errors.frontId}</p>}
          </div>
