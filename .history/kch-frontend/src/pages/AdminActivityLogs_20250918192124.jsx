...
function AdminActivityLogs() {
  ...
  useEffect(() => {
    if (userRole) {
      ...
      if (!isSuperAdmin) {
        setError("Access denied. Only super admins can view admin activity logs.");
      }
    }
  }, [userRole]);

  // Fetch admin activity logs
  const fetchLogs = async (page = 0, filters = searchFilters) => {
    if (!hasAccess) return;
    ...
    try {
      ...
      if (filters.adminEmail.trim()) {
        params.append("adminEmail", filters.adminEmail.trim());
      }
      if (filters.action.trim()) {
        params.append("action", filters.action.trim());
      }

      const response = await api.get(`/admin/admin-activity-logs?${params}`);
      ...
      setError(`Failed to fetch admin activity logs: ${error.response?.data || error.message}`);
  ...
  const getActionStyle = (action) => {
    if (action.includes("DELETE")) return { color: "text-red-400 bg-red-500/10", icon: <AlertTriangle className="w-4 h-4" /> };
    if (action.includes("APPROVE") || action.includes("ACTIVATE")) return { color: "text-green-400 bg-green-500/10", icon: <User className="w-4 h-4" /> };
    if (action.includes("REVOKE") || action.includes("SUSPEND")) return { color: "text-orange-400 bg-orange-500/10", icon: <AlertTriangle className="w-4 h-4" /> };
    if (action.includes("VIEW")) return { color: "text-blue-400 bg-blue-500/10", icon: <Eye className="w-4 h-4" /> };
    return { color: "text-gray-400 bg-gray-500/10", icon: <FileText className="w-4 h-4" /> };
  };
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-8 text-center">
            ...
            <p className="text-red-300">
              {error || "Only super administrators can access admin activity logs."}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-white/70">
                Monitor all administrative actions for accountability and security
              </p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSearchFilters(prev => ({ ...prev, adminEmail: e.target.value }))}
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="text-white/80 text-lg">Loading admin activity logs...</p>
          </div>
        ) : logs.length > 0 ? (
          <>
            {/* Summary */}
            <div className="mb-6 p-4 bg-slate-800/50 border border-slate-600 rounded-xl">
              <div className="flex items-center justify-between">
                <div className="text-white/70">
                  ...
                  {searchFilters.adminEmail || searchFilters.action ? " (filtered)" : ""}
                </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          <div className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-2 ${actionStyle.color}`}>
                            {actionStyle.icon}
                            {log.action}
                          </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                          <div>
                            <div className="text-white/60 text-xs mb-1">Admin</div>
                            <div className="text-white font-medium">
                              {log.adminName || "Unknown"} 
                              <span className="text-white/60 text-sm ml-2">({log.adminEmail})</span>
                            </div>
                            <div className="text-white/50 text-xs">Role: {log.adminRole}</div>
                          </div>
                          {log.targetUserId && (
                            <div>
                              <div className="text-white/60 text-xs mb-1">Target User</div>
                              <div className="text-white font-medium">
                                {log.targetUserName || "Unknown"}
                                <span className="text-white/60 text-sm ml-2">({log.targetUserEmail})</span>
                              </div>
                              <div className="text-white/50 text-xs">Role: {log.targetUserRole}</div>
                            </div>
                          )}
                        </div>
                        ...
                        {log.details && (
                          <div className="bg-slate-700/30 rounded-lg p-3">
                            <div className="text-white/60 text-xs mb-1">Details</div>
                            <div className="text-white/80 text-sm">{log.details}</div>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-white/80 text-lg mb-2">No admin activity logs found</p>
