  ...
  const reviewDocument = async (docId, verified, notes = "") => {
    ...
    try {
      ...
      if (
        ...
        if (verified) {
          // Approve user
          console.log(`🔄 Approving user ${userId} with admin ${user?.email}`);
          console.log(`📤 Request payload:`, {
            adminEmail: user?.email,
            userId: userId,
            approvalNotes: notes || `Document ${docId} approved by admin`,
          });

          const response = await api.post(`/user-approval/approve`, {
            adminEmail: user?.email,
            userId: userId,
            approvalNotes: notes || `Document ${docId} approved by admin`,
          });
          ...
          // Reject user
          console.log(`🔄 Rejecting user ${userId} with admin ${user?.email}`);
          console.log(`📤 Request payload:`, {
            adminEmail: user?.email,
            userId: userId,
            rejectionNotes: notes || `Document ${docId} rejected by admin`,
          });

          const response = await api.post(`/user-approval/reject`, {
            adminEmail: user?.email,
            userId: userId,
            rejectionNotes: notes || `Document ${docId} rejected by admin`,
          });
          console.log(`✅ User rejection response:`, response.data);
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
                      ...
                      {d.storageKey && (
                        <div className="mb-3">
                          <div 
                            ...
                            onClick={() => openDocumentViewer(d)}
                          >
                            <img
                              src={getImageUrl(d.storageKey)}
                              alt={d.docType}
                              className="w-full h-32 object-cover rounded-lg border border-slate-600 transition-all duration-200 group-hover:border-blue-500/50 group-hover:shadow-lg"
                              onError={(e) => {
                            ...
                            />
                            <div
                              className="hidden w-full h-32 bg-slate-700/50 rounded-lg border border-slate-600"
                              style={{ display: "none" }}
                            >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          {d.verified === true ? (
                            ...
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-red-500/20 text-red-300 rounded-full text-xs font-medium">
                              <XCircle className="w-3 h-3" />
                              Rejected
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-yellow-500/20 text-yellow-300 rounded-full text-xs font-medium">
                        ...
                        <div className="flex items-center gap-2">
                          {d.verified === null ? (
                            // Pending documents - show both approve and reject
                            <>
                              <button
                                onClick={() => openReviewModal(d, true)}
                                disabled={actionLoading.has(d.id)}
                            ...
                            // Reviewed documents - show update button
                            <button
                              onClick={() => openReviewModal(d, !d.verified)}
                              disabled={actionLoading.has(d.id)}
                              className="bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
      ...
      {showReviewModal && selectedDocument && (
        ...
        >
          <div
            className="bg-slate-800 rounded-2xl border border-slate-600 w-full max-w-md shadow-2xl backdrop-blur-md"
          ...
          >
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-white">
                      {selectedDocument.action ? "Approve" : "Reject"} Document
                    </h3>
                ...
                <button
                  onClick={closeReviewModal}
                  disabled={actionLoading.has(selectedDocument.id)}
                  className="p-2 hover:bg-slate-700/50 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600 rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                    rows={3}
                  />
                </div>
                ...
                <div className="flex items-center gap-3 pt-4">
                  <button
                    ...
                    disabled={actionLoading.has(selectedDocument.id)}
                    className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed ${
                      selectedDocument.action
                        ? "bg-green-500/20 hover:bg-green-500/30 text-green-300"
                        : "bg-red-500/20 hover:bg-red-500/30 text-red-300"
                    }`}
                  >
                    {actionLoading.has(selectedDocument.id) ? (
                      <>
                      ...
                      <>
                        ...
                        {selectedDocument.action ? "Approve" : "Reject"}{" "}
                        Document
                      </>
                    )}
                  </button>
                  <button
                    onClick={closeReviewModal}
                    disabled={actionLoading.has(selectedDocument.id)}
                    className="px-4 py-2 bg-slate-600/50 hover:bg-slate-600/70 text-white rounded-lg font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Cancel
                  </button>
                </div>
      ...
      {showDocumentViewer && viewingDocument && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 rounded-2xl border border-slate-600 max-w-6xl max-h-[90vh] w-full overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-slate-600">
              <div>
                ...
                <p className="text-white/70 text-sm">
                  Document for {users.find((u) => u.id === selectedUserId)?.fname}{" "}
                  {users.find((u) => u.id === selectedUserId)?.lname}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    ...
                    link.href = getImageUrl(viewingDocument.storageKey);
                    link.download = `${viewingDocument.docType}_${users.find((u) => u.id === selectedUserId)?.fname}_${users.find((u) => u.id === selectedUserId)?.lname}.jpg`;
                    link.click();
                  }}
            ...
            <div className="p-6 overflow-auto max-h-[calc(90vh-120px)]">
              <div className="flex justify-center">
                <img
                  src={getImageUrl(viewingDocument.storageKey)}
                  alt={viewingDocument.docType}
                  className="max-w-full max-h-[70vh] object-contain rounded-lg border border-slate-600 shadow-2xl"
                  onError={(e) => {
                    e.target.style.display = "none";
                ...
                />
                <div
                  className="hidden w-full h-96 bg-slate-700/50 rounded-lg border border-slate-600 items-center justify-center"
                  style={{ display: "none" }}
                >
                  <div className="text-center">
                    <FileText className="w-16 h-16 text-white/30 mx-auto mb-4" />
                    <p className="text-white/70">Failed to load document</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Document Info */}
            <div className="p-6 border-t border-slate-600 bg-slate-700/30">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  <span className="text-white/60 font-medium">File Type:</span>
                  <div className="text-white">Image (JPG/PNG)</div>
                </div>
              </div>
              
              {viewingDocument.reviewNotes && (
                <div className="mt-4">
                  <span className="text-white/60 font-medium">Review Notes:</span>
                  <div className="text-white mt-1 p-3 bg-slate-600/50 rounded-lg">
                    {viewingDocument.reviewNotes}
                  </div>
                </div>
              )}
