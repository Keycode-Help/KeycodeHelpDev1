...
function AdminActivityLogs() {
  ...
  const fetchLogs = async (
    ...
    try {
      ...
      // Try the test endpoint first if this is a retry to show sample data
      if (retryCount > 0) {
        console.log("🧪 Trying test admin logs endpoint as fallback...");
        try {
          response = await api.get("/admin/admin-logs-test", {
            timeout: 10000, // 10 second timeout for test endpoint
          });
          console.log("✅ Test endpoint worked, using fallback response with sample data");
        } catch (testError) {
          console.log(
            "❌ Test endpoint also failed, falling back to simple endpoint"
          );
