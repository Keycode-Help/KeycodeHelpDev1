...
function NastfCompliance() {
  ...
  useEffect(() => {
    if (userRole) {
      // Temporary workaround: Grant admin access to known admin email
      const userEmail = user?.email;
      const isKnownAdmin = userEmail === "keycodehelp@gmail.com";
      
      let effectiveRole = userRole;
      if (isKnownAdmin && userRole === "BASEUSER") {
        console.log("🔧 Known admin detected with BASEUSER role, granting admin access...");
        effectiveRole = "ADMIN";
        console.warn("⚠️ Temporary admin access granted - please update user role in database");
      }

      const hasAccess = canSeeAdmin(effectiveRole);
      setHasAdminAccess(hasAccess);

      console.log("NastfCompliance - Admin access check:", {
        userRole,
        effectiveRole,
        hasAccess,
        userEmail,
  ...
  const fetchComplianceData = async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/admin/nastf-compliance${filter !== "all" ? `?status=${filter}` : ""}`
      );
