...
function UserManagement() {
  ...
  const fetchAllUsers = useCallback(async () => {
    ...
    try {
      ...
      const usersWithStatus = users.map(user => ({
        ...
        displayName: `${user.fname || ''} ${user.lname || ''}`.trim() || 'Unknown User'
      }));
  ...
  const isTrialActive = (user) => {
    return user.trialActive && user.trialEndsAt && new Date(user.trialEndsAt) > new Date();
  };
  ...
  const handleActivateTrial = (userId) => handleUserAction(userId, "activate-trial", { trialDays: 3 });
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-white/70">Manage all users, trials, and subscriptions</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {stats.map((stat, index) => (
              <div key={index} className={`bg-gradient-to-br ${stat.color} border rounded-2xl p-6 text-center shadow-lg`}>
                <div className="flex items-center justify-center mb-2">
                  {stat.icon}
                </div>
                <div className="text-2xl font-bold text-white mb-1">{stat.value}</div>
                <div className="text-sm font-medium text-white/70">{stat.title}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-800 rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-6">
                ...
                {/* Documents */}
                {(selectedUser.frontId || selectedUser.backId || selectedUser.insurance) && (
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {selectedUser.frontId && (
                        <div>
                          ...
                          />
                          <p className="text-white/70 text-sm mt-2 text-center">Front ID</p>
                        </div>
                      )}
                      {selectedUser.backId && (
                        <div>
                          <img
                            ...
                            className="w-full h-32 object-cover rounded-lg border border-slate-600"
                          />
                          <p className="text-white/70 text-sm mt-2 text-center">Back ID</p>
                        </div>
                      )}
                      {selectedUser.insurance && (
                        <div>
                          <img
                            ...
                            className="w-full h-32 object-cover rounded-lg border border-slate-600"
                          />
                          <p className="text-white/70 text-sm mt-2 text-center">Insurance</p>
                        </div>
                      )}
                    </div>
                  </div>
