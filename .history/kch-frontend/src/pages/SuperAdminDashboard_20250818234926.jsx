...
import { Shield, CheckCircle2, XCircle, Search, Trash2, Mail } from "lucide-react";
...
function SuperAdminDashboard() {
  ...
  const [codeForm, setCodeForm] = useState({ email: "", firstName: "", lastName: "" });
  ...
  const handleApprove = async (adminId, notes) => {
    try {
      ...
      setActionLog((l) => [{ t: Date.now(), msg: `Approved admin ID ${adminId}` }, ...l]);
      fetchAdminData();
    } catch (error) {
  ...
  const handleReject = async (adminId, notes) => {
    try {
      ...
      setActionLog((l) => [{ t: Date.now(), msg: `Rejected admin ID ${adminId}` }, ...l]);
      fetchAdminData();
    } catch (error) {
  ...
  const bulkApprove = async () => {
    ...
    setActionLog((l) => [{ t: Date.now(), msg: `Bulk approved ${ids.length} admins` }, ...l]);
    setSelected(new Set());
    fetchAdminData();
  };
  ...
  const bulkReject = async () => {
    ...
    setActionLog((l) => [{ t: Date.now(), msg: `Bulk rejected ${ids.length} admins` }, ...l]);
    setSelected(new Set());
    fetchAdminData();
  };
  ...
  const requestAdminCode = async (e) => {
    ...
    try {
      ...
      setActionLog((l) => [{ t: Date.now(), msg: `Sent admin code to ${email}` }, ...l]);
    ...
    <div className="relative">
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6 py-10 relative">
        ...
        <div className="mb-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="glass-card rounded-2xl border border-[var(--border)] p-4 flex items-center gap-3">
            <button
              ...
              className={`rounded-xl px-4 py-2 font-medium transition ${selected.size ? "bg-emerald-500/90 hover:bg-emerald-500 text-white" : "bg-gray-700 text-gray-400 cursor-not-allowed"}`}
            >
            ...
            <button
              onClick={bulkReject}
              disabled={!selected.size}
              className={`rounded-xl px-4 py-2 font-medium transition ${selected.size ? "bg-red-500/90 hover:bg-red-500 text-white" : "bg-gray-700 text-gray-400 cursor-not-allowed"}`}
            >
              Bulk Reject
            </button>
          </div>
          <form onSubmit={requestAdminCode} className="glass-card rounded-2xl border border-[var(--border)] p-4 grid grid-cols-1 md:grid-cols-4 gap-2 items-center">
            <div className="flex items-center gap-2 md:col-span-4">
              <Mail className="h-5 w-5 text-gray-400" />
              <span className="text-white font-medium">Send Admin Registration Code</span>
            </div>
            <input value={codeForm.email} onChange={(e) => setCodeForm((f) => ({ ...f, email: e.target.value }))} placeholder="Email" className="rounded-lg bg-black/30 border border-[var(--border)] text-white placeholder-gray-400 p-2 focus:outline-none focus:ring-2 focus:ring-yellow-500" required />
            <input value={codeForm.firstName} onChange={(e) => setCodeForm((f) => ({ ...f, firstName: e.target.value }))} placeholder="First name" className="rounded-lg bg-black/30 border border-[var(--border)] text-white placeholder-gray-400 p-2 focus:outline-none focus:ring-2 focus:ring-yellow-500" required />
            <input value={codeForm.lastName} onChange={(e) => setCodeForm((f) => ({ ...f, lastName: e.target.value }))} placeholder="Last name" className="rounded-lg bg-black/30 border border-[var(--border)] text-white placeholder-gray-400 p-2 focus:outline-none focus:ring-2 focus:ring-yellow-500" required />
            <button type="submit" className="rounded-xl bg-yellow-500/90 hover:bg-yellow-500 text-black font-semibold px-4 py-2">Send Code</button>
          </form>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="glass-card rounded-3xl border border-[var(--border)] p-5">
            ...
            {pendingAdmins.filter((f) => {
              ...
              if (!q) return true;
              const s = `${f.fname} ${f.lname} ${f.email} ${f.company}`.toLowerCase();
              return s.includes(q);
            }).length === 0 ? (
        ...
        <div className="mt-6 glass-card rounded-3xl border border-[var(--border)] p-5">
          <h2 className="text-white font-semibold text-lg mb-3">Recent Actions</h2>
          {actionLog.length === 0 ? (
