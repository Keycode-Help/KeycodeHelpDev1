...
function AdminDashboard() {
  ...
  const getImageUrl = useCallback((imageData) => {
    ...
    if (optimizedUrl && optimizedUrl.startsWith('blob:')) {
      setBlobUrls(prev => {
        // Only update if this imageData isn't already tracked
        if (!prev.has(imageData)) {
          const newMap = new Map(prev);
          newMap.set(imageData, optimizedUrl);
          return newMap;
        }
        return prev;
      });
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2 mt-auto">
                          <img
                            ...
                            onClick={() => openModal(getImageUrl(request.frontId))}
                            onError={(e) => {
                              e.target.style.display = "none";
                            }}
                          />
                          ...
                          <img
                            ...
                            alt="Registration"
                            className="w-20 h-20 object-cover rounded-lg border border-slate-600 hover:border-slate-500 cursor-pointer transition-colors duration-200 flex-shrink-0"
                            onClick={() => openModal(getImageUrl(request.registration))}
                            onError={(e) => {
                              e.target.style.display = "none";
                            }}
                          />
                          ...
                          <div className="group">
                            <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-600 hover:border-slate-500 transition-all duration-300 shadow-lg hover:shadow-xl">
                              <img
                                ...
                                onClick={() => openModal(getImageUrl(user.frontId))}
                                onError={(e) =>
                          ...
                          <div className="group">
                            <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-600 hover:border-slate-500 transition-all duration-300 shadow-lg hover:shadow-xl">
                              <img
                                ...
                                alt="Back ID"
                                className="w-full h-40 object-cover rounded-xl border border-slate-600 group-hover:border-slate-500 cursor-pointer transition-colors duration-200"
                                onClick={() => openModal(getImageUrl(user.backId))}
                                onError={(e) => {
                                  e.target.style.display = "none";
                          ...
                          <div className="group">
                            <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-600 hover:border-slate-500 transition-all duration-300 shadow-lg hover:shadow-xl">
                              <img
                                ...
                                alt="Insurance"
                                className="w-full h-40 object-cover rounded-xl border border-slate-600 group-hover:border-slate-500 cursor-pointer transition-colors duration-200"
                                onClick={() => openModal(getImageUrl(user.insurance))}
                                onError={(e) => {
                                  e.target.style.display = "none";
                                  e.target.nextSibling.style.display = "block";
                                }}
