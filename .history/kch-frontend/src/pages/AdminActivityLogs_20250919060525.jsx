...
function AdminActivityLogs() {
  ...
  const generateTestLogs = async () => {
    ...
    try {
      const response = await api.post("/admin/generate-test-logs");
  ...
  const fetchLogs = async (page = 0, filters = searchFilters, retryCount = 0) => {
    if (!hasAccess) return;
    ...
    try {
      ...
      if (error.response?.status === 403) {
        setError("Session expired. Please log in again to access admin activity logs.");
        ...
        console.log(`⏱️ Request timed out, retrying... (attempt ${retryCount + 1}/3)`);
