...
function UserDash() {
  ...
  const handleUpdateRequest = (requestId) => {
    ...
    if (requestFiles.registration) formData.append("registration", requestFiles.registration);

    axios
      .put(`http://localhost:8080/vehicle/update-request/${requestId}`, formData, {
        headers: {
    ...
    const filter = (arr) =>
      !q
        ...
        : arr.filter((r) =>
            ...
            ((r.make && r.make.manufacturerName) || "").toLowerCase().includes(q)
          );
    return {
    ...
    return (
      <div className="rounded-xl border border-gray-800 bg-[#111318] p-4 shadow-sm hover:shadow-md transition-shadow">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-lg font-semibold text-white">{makeName} • {request.model}</h3>
            <p className="text-sm text-gray-400 mt-1">VIN: {request.vin}</p>
            <p className="text-sm text-gray-400">Price: ${request.keycodePrice}</p>
            <span className={`mt-2 inline-block rounded-full px-2.5 py-0.5 text-xs ${
              status === "pending" ? "bg-yellow-500/10 text-yellow-400 border border-yellow-500/30" :
              status === "inProgress" ? "bg-blue-500/10 text-blue-400 border border-blue-500/30" :
              "bg-green-500/10 text-green-400 border border-green-500/30"
            }`}>
              {status === "pending" ? "Pending" : status === "inProgress" ? "In Progress" : "Fulfilled"}
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              className="inline-flex items-center gap-1 rounded-md border border-gray-700 px-3 py-1.5 text-xs text-gray-200 hover:bg-gray-800"
              onClick={() =>
                setUpdateData((prev) => ({ ...prev, [request.id]: prev[request.id] || { model: request.model, vin: request.vin } }))
              }
            >
              <Edit2 size={16} /> Edit
            </button>
        ...
        {editing && (
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">Front ID</label>
              <input
                type="file"
                name="frontId"
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">Back ID</label>
              <input
                type="file"
                name="backId"
                onChange={(e) => handleFileChange(e, request.id)}
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 md:px-6">
      <header className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">Welcome{user?.fname ? `, ${user.fname}` : ""}</h1>
          <p className="text-sm text-gray-400">Track and manage your keycode requests.</p>
        </div>
      ...
      <section className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl border border-gray-800 bg-[#111318] p-4">
          ...
          <p className="mt-1 text-2xl font-bold text-white">{stats.inProgress}</p>
        </div>
        <div className="rounded-xl border border-gray-800 bg-[#111318] p-4">
          <p className="text-sm text-gray-400">Fulfilled</p>
          <p className="mt-1 text-2xl font-bold text-white">{stats.fulfilled}</p>
        </div>
      </section>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <TabButton id="inProgress" label="In Progress" count={stats.inProgress} />
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {(activeTab === "pending" ? filtered.pending : activeTab === "inProgress" ? filtered.inProgress : filtered.fulfilled).map(
            (req) => (
