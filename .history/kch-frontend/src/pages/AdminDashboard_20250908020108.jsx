...
function AdminDashboard() {
        ...
        if (response.data && response.data.length > 0) {
          response.data.forEach((request, index) => {
            console.log(`🚗 Request ${index} image data:`, {
              frontId: request.frontId ? `${request.frontId.substring(0, 50)}...` : 'null',
              backId: request.backId ? `${request.backId.substring(0, 50)}...` : 'null',
              registration: request.registration ? `${request.registration.substring(0, 50)}...` : 'null',
              frontIdLength: request.frontId ? request.frontId.length : 0,
              backIdLength: request.backId ? request.backId.length : 0,
              registrationLength: request.registration ? request.registration.length : 0
            });
          });
        }
        ...
        if (response.data && response.data.length > 0) {
          response.data.forEach((user, index) => {
            console.log(`📸 User ${index} image data:`, {
              frontId: user.frontId ? `${user.frontId.substring(0, 50)}...` : 'null',
              backId: user.backId ? `${user.backId.substring(0, 50)}...` : 'null',
              insurance: user.insurance ? `${user.insurance.substring(0, 50)}...` : 'null',
  ...
  const openModal = (imageSrc) => {
    console.log("🖼️ Opening modal with image:", imageSrc ? imageSrc.substring(0, 100) + "..." : "null");
                          ...
                          <div className="group">
                            <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-600 hover:border-slate-500 transition-all duration-300 shadow-lg hover:shadow-xl">
                              <img
                                ...
                                onError={(e) => {
                                  console.error("❌ Front ID image failed to load:", user.frontId ? user.frontId.substring(0, 100) + "..." : "null");
                                  e.target.style.display = "none";
                                ...
                                onLoad={() => {
                                  console.log("✅ Front ID image loaded successfully");
                                }}
                              />
                              <div className="hidden w-full h-40 bg-slate-700/50 rounded-xl border border-slate-600 flex items-center justify-center">
                                <div className="text-center">
                                  <p className="text-white/40 text-sm mb-2">Image failed to load</p>
                                  <p className="text-white/20 text-xs">Check console for details</p>
                                </div>
                              </div>
