...
function UserDash() {
  ...
  const handleUpdateRequest = (requestId) => {
    ...
    api
      .put(`/vehicle/update-request/${requestId}`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      })
      .then(() => {
        return api.get("/vehicle/user-requests");
      })
      .then((response) => setRequests(response.data))
      .catch((error) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-dark via-secondary to-dark">
      <div className="mx-auto w-full max-w-6xl px-4 md:px-6 pb-16 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-full md:w-72">
              ...
              <input
                ...
                className="w-full rounded-full border border-secondary bg-surface/50 py-2 pl-9 pr-4 text-sm text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
          </div>
        </header>
        ...
        {/* Live chat visible only for premium/trial users */}
        <LiveChat visible={isPremium} />
      </div>
    </div>
  );
}
