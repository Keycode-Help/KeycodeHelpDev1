...
function VehicleKeycodeRequest() {
  ...
  const [isVehicleNotAvailable, setIsVehicleNotAvailable] = useState(false);
  ...
  useEffect(() => {
    ...
    console.log("🚗 Available vehicle makes:", makes.length, makes);
  ...
  const setMakePrice = (selectedMakeValue) => {
    if (selectedMakeValue) {
      ...
      const availNote = getVehicleAvailabilityNote(selectedMakeValue);
      const vehicleAvailable = isVehicleAvailable(selectedMakeValue);
      const limitedStock = hasLimitedAvailability(selectedMakeValue);

      setSelectedMakePrice(nonMemberPrice);
      setSelectedMemberPrice(memberPrice);
      setSelectedCategory(category);
      setRequiresPinCode(needsPin);
      setAvailabilityNote(availNote || "");
      setIsVehicleNotAvailable(!vehicleAvailable);
      setHasLimitedStock(limitedStock);
    } else {
      ...
      setAvailabilityNote("");
      setIsVehicleNotAvailable(false);
      setHasLimitedStock(false);
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-dark via-secondary to-dark p-4 relative">
      {/* Background Effects */}
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-40 pointer-events-none"></div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm border border-slate-700 rounded-2xl p-8 shadow-2xl overflow-visible">
          {/* Header */}
          ...
          <div className="bg-gradient-to-r from-slate-700/20 to-slate-800/20 border border-slate-600/50 rounded-xl p-4 mb-6">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-gray-300 leading-relaxed">
                <p>
                  <strong className="text-white">Availability Notice:</strong> Some keycodes may not be available due to vehicle year, model variations, or data limitations. In cases where we cannot provide your requested keycode, you will be eligible for a full refund. We'll notify you within 24-48 hours if your specific vehicle cannot be serviced.
                </p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-white mb-3">
                  ...
                  <span className="text-xs text-slate-400 ml-2">
                    ({availableMakes.length} available)
                  </span>
                </label>
                <MakeDropDown
            ...
            {selectedMake && isVehicleNotAvailable && (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-red-400 mb-2">
                      {selectedMake} - Keycodes Not Available
                    </h3>
                    <p className="text-red-300">
                      {availabilityNote ||
                        "Keycodes for this vehicle brand are not currently available."}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {selectedMake && !isVehicleNotAvailable && hasLimitedStock && (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-yellow-400 mb-2">
                      {selectedMake} - Limited Availability
                    </h3>
                    <p className="text-yellow-300">
                      {availabilityNote ||
                        "Limited keycode availability for this brand. Please contact support for specific models."}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full px-4 py-3 bg-slate-800/50 border border-slate-600 rounded-xl text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={isVehicleNotAvailable}
              className={`w-full font-semibold py-3 px-6 rounded-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 transition-all duration-200 flex items-center justify-center gap-2 ${
                isVehicleNotAvailable
                  ...
                  : "bg-gradient-to-r from-blue-500 to-yellow-500 text-white hover:from-blue-600 hover:to-yellow-600 focus:ring-blue-500"
              }`}
            >
              <Icon
                name={isVehicleNotAvailable ? "x" : "shoppingCart"}
                size={18}
              />
              {isVehicleNotAvailable
                ? "Not Available"
