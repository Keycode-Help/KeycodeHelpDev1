...
function AdminActivityLogs() {
  ...
  const fetchLogs = async (
    ...
    try {
      ...
      // Try the simple endpoint first if this is a retry
      if (retryCount > 0) {
        console.log("🧪 Trying simple admin logs endpoint as fallback...");
        try {
          response = await api.get("/admin/admin-logs-simple", {
            timeout: 10000, // 10 second timeout for simple endpoint
          });
          console.log("✅ Simple endpoint worked, using fallback response");
        } catch (simpleError) {
          console.log("❌ Simple endpoint also failed, falling back to original");
          throw simpleError;
        }
      } else {
