...
export const debugAuthState = () => {
  ...
  console.log(
    "- auth_token:",
    sessionToken ? `✅ Present (${sessionToken.length} chars)` : "❌ Missing"
  );
  ...
  if (!authToken && !cookies.access_token) {
    ...
    console.log(
      "1. ⚠️ Token in localStorage but not in cookies - may cause issues"
    );
    console.log("   Try logging out and logging back in");
  } else if (!authToken && cookies.access_token) {
    console.log(
      "1. ⚠️ Token in cookies but not in localStorage - inconsistent state"
    );
  }
