...
export const getOptimizedImageUrl = (imageData, maxBase64Length = 100000) => {
  if (!imageData) {
    console.log("🔍 getOptimizedImageUrl: No image data provided");
    return null;
  }

  console.log("🔍 getOptimizedImageUrl: Processing", {
    length: imageData.length,
    startsWith: imageData.substring(0, 20),
    isHttp: imageData.startsWith("http"),
    isSlash: imageData.startsWith("/"),
    isData: imageData.startsWith("data:")
  });

  // If it's already a regular URL (not base64), return as is
  if (imageData.startsWith("http")) {
    console.log("🔍 getOptimizedImageUrl: Returning HTTP URL");
    return imageData;
  }
  ...
  if (
    ...
    console.log("🔍 getOptimizedImageUrl: Returning relative path");
    return imageData;
  }
  ...
  if (base64String.length > maxBase64Length) {
    console.log("🔍 getOptimizedImageUrl: Converting to blob URL (large image)");
    const blobUrl = base64ToBlobUrl(base64String, mimeType);
    console.log("🔍 getOptimizedImageUrl: Blob URL created", blobUrl ? "success" : "failed");
    return blobUrl;
  } else {
    // For smaller base64 strings, create data URL
    console.log("🔍 getOptimizedImageUrl: Creating data URL (small image)");
    const dataUrl = `data:${mimeType};base64,${base64String}`;
    console.log("🔍 getOptimizedImageUrl: Data URL created", dataUrl.substring(0, 50) + "...");
    return dataUrl;
  }
};
