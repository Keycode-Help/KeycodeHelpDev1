...
-- Migration: Update database structure from old format to new format

-- Step 1: Add new columns to make table
ALTER TABLE make ADD COLUMN IF NOT EXISTS non_member_price DECIMAL(10,2);
ALTER TABLE make ADD COLUMN IF NOT EXISTS member_price DECIMAL(10,2);

-- Step 2: Set default values for the new pricing columns
-- Since we're migrating from an existing database, we'll set reasonable defaults
UPDATE make SET 
