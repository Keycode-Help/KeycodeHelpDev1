package org.rma.kchbackend.util;

import org.rma.kchbackend.service.CustomUserDetailsService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.security.authentication.UsernamePasswordAuthenticationToken;
import org.springframework.security.core.GrantedAuthority;
import org.springframework.security.core.authority.SimpleGrantedAuthority;
import org.springframework.security.core.context.SecurityContextHolder;
import org.springframework.security.core.userdetails.UserDetails;
import org.springframework.security.web.authentication.WebAuthenticationDetailsSource;
import org.springframework.stereotype.Component;
import org.springframework.web.filter.OncePerRequestFilter;
import io.jsonwebtoken.Claims;
import java.util.Collection;
import java.util.List;
import java.util.stream.Collectors;
import org.springframework.lang.NonNull;

import jakarta.servlet.FilterChain;
import jakarta.servlet.ServletException;
import jakarta.servlet.http.Cookie;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;
import java.io.IOException;

@Component
public class JwtAuthenticationFilter extends OncePerRequestFilter {

    @Autowired
    private CustomUserDetailsService userDetailsService;

    @Autowired
    private JwtUtil jwtUtil;

    @Override
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain chain)
            throws ServletException, IOException {
        
        String requestURI = request.getRequestURI();
        String requestMethod = request.getMethod();
        
        // Debug logging
        System.out.println("🔍 JWT Filter - Request URI: " + requestURI);
        System.out.println("🔍 JWT Filter - Request Method: " + requestMethod);
        System.out.println("🔍 JWT Filter - Authorization Header: " + request.getHeader("Authorization"));

        // Skip JWT processing for public endpoints
        boolean isPublic = isPublicEndpoint(requestURI, requestMethod);
        System.out.println("JWT Filter - isPublicEndpoint(" + requestURI + ", " + requestMethod + ") = " + isPublic);
        if (isPublic) {
            System.out.println("JWT Filter - Skipping JWT processing for public endpoint: " + requestURI);
            chain.doFilter(request, response);
            return;
        }
        
        System.out.println("JWT Filter - Processing protected endpoint: " + requestURI);

        String username = null;
        String jwt = null;

        // First try to get JWT from Authorization header
        final String authorizationHeader = request.getHeader("Authorization");
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            jwt = authorizationHeader.substring(7);
            System.out.println("JWT Filter - Found JWT in Authorization header");
        } else {
            // If no Authorization header, try to get JWT from cookies
            Cookie[] cookies = request.getCookies();
            if (cookies != null) {
                System.out.println("JWT Filter - Cookies found: " + cookies.length);
                for (Cookie cookie : cookies) {
                    System.out.println("JWT Filter - Cookie: " + cookie.getName() + " = " + 
                        (cookie.getName().contains("token") ? "[TOKEN_VALUE]" : cookie.getValue()));
                }
                for (Cookie cookie : cookies) {
                    // Check for access_token first, then next-auth.session-token
                    if ("access_token".equals(cookie.getName()) || "next-auth.session-token".equals(cookie.getName())) {
                        jwt = cookie.getValue();
                        System.out.println("JWT Filter - Found JWT in cookie: " + cookie.getName());
                        System.out.println("JWT Filter - Token length: " + (jwt != null ? jwt.length() : 0));
                        System.out.println("JWT Filter - Token starts with: " + (jwt != null && jwt.length() > 20 ? jwt.substring(0, 20) + "..." : jwt));
                        break;
                    }
                }
            } else {
                System.out.println("JWT Filter - No cookies found in request");
            }
            if (jwt == null) {
                System.out.println("JWT Filter - No JWT found in headers or cookies");
            }
        }

        if (jwt != null) {
            // Check if JWT is well-formed
            if (!jwt.contains(".")) {
                System.out.println("JWT Filter - Invalid JWT format - token does not contain dots");
                System.out.println("JWT Filter - Token value: " + jwt);
                response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Invalid JWT format.");
                return;
            }

            // Check if it's a valid JWT structure (3 parts separated by dots)
            String[] parts = jwt.split("\\.");
            if (parts.length != 3) {
                System.out.println("JWT Filter - Invalid JWT structure - expected 3 parts, got " + parts.length);
                System.out.println("JWT Filter - Token value: " + jwt);
                response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Invalid JWT structure.");
                return;
            }

            try {
                username = jwtUtil.extractUsername(jwt);
                System.out.println("JWT Filter - Extracted username: " + username);
            } catch (Exception e) {
                System.out.println("JWT Filter - Error extracting username: " + e.getMessage());
                System.out.println("JWT Filter - Token that failed: " + jwt);
                System.out.println("JWT Filter - This might be a Supabase JWT token that needs proper secret configuration");
                // Don't return error here, just log and continue - this might be a Supabase token
                // that we can't parse with the current secret, but we should still allow the request to continue
            }
        }

        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            try {
                UserDetails userDetails = this.userDetailsService.loadUserByUsername(username);
                
                // Check if this is a Supabase JWT token (doesn't contain our custom claims)
                boolean isSupabaseToken = false;
                try {
                    Claims claims = jwtUtil.extractAllClaims(jwt);
                    isSupabaseToken = !claims.containsKey("authorities");
                    System.out.println("JWT Filter - Is Supabase token: " + isSupabaseToken);
                } catch (Exception e) {
                    isSupabaseToken = true; // If we can't parse it with our secret, it's likely a Supabase token
                    System.out.println("JWT Filter - Assuming Supabase token due to parsing error: " + e.getMessage());
                }
                
                boolean isValidToken = false;
                if (isSupabaseToken) {
                    // For Supabase tokens, we trust the token and just validate the user exists
                    System.out.println("JWT Filter - Trusting Supabase JWT token for user: " + username);
                    isValidToken = true;
                } else {
                    // For our own JWT tokens, validate normally
                    isValidToken = jwtUtil.validateToken(jwt, userDetails);
                    System.out.println("JWT Filter - Validated our JWT token: " + isValidToken);
                }
                
                if (isValidToken) {
                    // Extract authorities from JWT token if available
                    Collection<? extends GrantedAuthority> authorities = userDetails.getAuthorities();
                    try {
                        // Try to get authorities from JWT token claims
                        Claims claims = jwtUtil.extractAllClaims(jwt);
                        if (claims.containsKey("authorities")) {
                            @SuppressWarnings("unchecked")
                            List<String> authorityStrings = (List<String>) claims.get("authorities");
                            authorities = authorityStrings.stream()
                                    .map(authority -> new SimpleGrantedAuthority(authority))
                                    .collect(Collectors.toList());
                            System.out.println("JWT Filter - Using authorities from token: " + authorities);
                        }
                    } catch (Exception e) {
                        System.out.println("JWT Filter - Could not extract authorities from token, using from UserDetails: " + e.getMessage());
                    }
                    
                    UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new UsernamePasswordAuthenticationToken(
                            userDetails, null, authorities);
                    usernamePasswordAuthenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
                    SecurityContextHolder.getContext().setAuthentication(usernamePasswordAuthenticationToken);
                    System.out.println("JWT Filter - Authentication successful for user: " + username + " with authorities: " + authorities);
                } else {
                    System.out.println("JWT Filter - Token validation failed for user: " + username);
                }
            } catch (Exception e) {
                System.out.println("JWT Filter - Error during authentication: " + e.getMessage());
                e.printStackTrace();
            }
        } else if (jwt != null && username == null) {
            System.out.println("JWT Filter - JWT token found but could not extract username");
        } else if (jwt == null) {
            System.out.println("JWT Filter - No JWT token found in request");
        }
        chain.doFilter(request, response);
    }

    /**
     * Check if the endpoint is public and should skip JWT processing
     */
    private boolean isPublicEndpoint(String requestURI, String requestMethod) {
        // Public endpoints that don't require JWT
        if (requestURI.equals("/") ||  // Only the root path, not all paths starting with /
            requestURI.startsWith("/auth/login") || 
            requestURI.startsWith("/auth/register") || 
            requestURI.startsWith("/auth/admin-register") ||
            requestURI.startsWith("/auth/refresh") ||
            requestURI.startsWith("/auth/reset-password") ||
            requestURI.startsWith("/auth/test") ||  // Allow test endpoints for debugging
            requestURI.startsWith("/auth/dev-") ||  // Allow dev endpoints
            requestURI.startsWith("/auth/setup-super-admin") ||
            requestURI.startsWith("/actuator/health") ||
            requestURI.startsWith("/css/") ||
            requestURI.startsWith("/js/") ||
            requestURI.startsWith("/images/") ||
            requestURI.startsWith("/makes/") ||
            requestURI.startsWith("/compliance/status") ||
            requestURI.startsWith("/cart/items/public") ||
            requestURI.startsWith("/api/payments/create-checkout-session") ||
            requestURI.startsWith("/api/kch/") ||
            requestURI.startsWith("/admin/admin-logs-public") ||
            requestURI.startsWith("/admin/admin-logs-test") ||
            requestURI.startsWith("/admin/admin-logs-simple")) {  // Allow public access to transponder database endpoints and admin test endpoints
            return true;
        }
        
        // OPTIONS requests (CORS preflight)
        if ("OPTIONS".equals(requestMethod)) {
            return true;
        }
        
        return false;
    }
}
