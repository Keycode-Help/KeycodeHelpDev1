...
public class AdminApprovalService {
    ...
    /**
     * Grant trial approval to an admin account (3 days access)
     */
    public boolean trialApproveAdminAccount(Long adminId, String approvalNotes, String superAdminEmail) {
        Optional<KeycodeUser> adminUser = keycodeUserRepository.findById(adminId);
        ...
        if (adminUser.isPresent()) {
            ...
            // Set trial approval - activate account and set trial end date (3 days from now)
            user.setAdminApproved(true);
            user.setAdminApprovalNotes(approvalNotes);
            user.setActive(true);
            ...
            // Log the trial approval action
            System.out.println("Trial approval granted by: " + superAdminEmail + " for user: " + user.getEmail() + " - 3 days access");
