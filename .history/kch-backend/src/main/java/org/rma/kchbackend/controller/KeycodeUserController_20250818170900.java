...
public class KeycodeUserController {
    ...
    public ResponseEntity<?> getUserSubscription(Authentication authentication) {
        String email = authentication.getName();
        KeycodeUser user = keycodeUserService.findByEmail(email)
                .orElseThrow(() -> new RuntimeException("User not found"));
        ...
        resp.put("tier", s.getTier());
        resp.put("activated", s.isActivated());
        resp.put("trial", s.isTrial());
        resp.put("trialEndsAt", s.getTrialEndsAt());
