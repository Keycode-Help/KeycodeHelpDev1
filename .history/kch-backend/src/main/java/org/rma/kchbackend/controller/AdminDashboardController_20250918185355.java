...
public class AdminDashboardController {
    ...
    public ResponseEntity<?> getNastfComplianceData(@RequestParam(required = false) String status, Authentication auth) {
        ...
        try {
            System.out.println("✅ Returning simple test response");
            return ResponseEntity.ok("NASTF Compliance endpoint is working! Auth: " + auth.getName() + " with roles: " + auth.getAuthorities());
        } catch (Exception e) {
            System.out.println("❌ Error in simple response: " + e.getMessage());
            return ResponseEntity.status(500).body("Error: " + e.getMessage());
        }
        
        // Check if this is the known super admin
        if (auth != null && "5epmgllc@gmail.com".equals(auth.getName())) {
            System.out.println("✅ Known super admin email detected");
            
            // Check authorities
            boolean hasAdminRole = auth.getAuthorities().stream()
                .anyMatch(authority -> authority.getAuthority().equals("ROLE_ADMIN") || 
                                     authority.getAuthority().equals("ROLE_SUPER_ADMIN"));
            System.out.println("  - Has Admin/Super Admin Role: " + hasAdminRole);
            
            if (!hasAdminRole) {
                System.out.println("⚠️ Super admin email but missing admin authorities!");
        ...
        try {
            ...
            if ("urgent".equals(status)) {
                // Get transactions that are within 1 day of 4-day deadline
                transactions = transactionService.getAllTransactions().stream()
                    .filter(t -> {
                        if (t.getVehicles() != null && !t.getVehicles().isEmpty()) {
                            Vehicle firstVehicle = t.getVehicles().get(0);
                            if (firstVehicle.getCreatedAt() != null) {
                                long daysSinceOrder = java.time.Duration.between(
                                    firstVehicle.getCreatedAt().atZone(java.time.ZoneId.systemDefault()).toInstant(),
                                    java.time.Instant.now()
                                ).toDays();
                                return daysSinceOrder >= 3 && daysSinceOrder < 4; // Day 3-4, urgent
                            }
                        }
                        return false;
                    })
                ...
                // Get all transactions within 4-day window
                transactions = transactionService.getAllTransactions().stream()
                    .filter(t -> {
                        if (t.getVehicles() != null && !t.getVehicles().isEmpty()) {
                            Vehicle firstVehicle = t.getVehicles().get(0);
                            if (firstVehicle.getCreatedAt() != null) {
                                long daysSinceOrder = java.time.Duration.between(
                                    firstVehicle.getCreatedAt().toInstant(),
                                    java.time.Instant.now()
                                ).toDays();
                                return daysSinceOrder < 4; // Within 4 days
                            }
                        }
                        return false;
                    })
            ...
            for (Transaction transaction : transactions) {
                for (Vehicle vehicle : transaction.getVehicles()) {
                    ...
                    data.put("status", vehicle.getStatus());
                    data.put("keycode", vehicle.getKeycode());
                    data.put("createdAt", vehicle.getCreatedAt());
            ...
            return ResponseEntity.status(500).body("Error fetching NASTF compliance data: " + e.getMessage());
        }
    }
