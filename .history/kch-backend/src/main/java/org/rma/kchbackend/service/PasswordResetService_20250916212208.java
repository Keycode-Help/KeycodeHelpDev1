...
public class PasswordResetService {
    ...
    public boolean sendPasswordResetEmail(String email, String resetUrl) {
        try {
            ...
            boolean emailSent = brevoEmailService.sendPasswordResetEmail(email, resetUrl, token);
            
            if (!emailSent) {
                System.out.println("⚠️ Brevo API failed, trying SMTP fallback...");
                ...
                System.out.println("🔧 SMTP Environment Configuration:");
                ...
                System.out.println("✅ Email sent successfully via SMTP fallback!");
            }
            return true;
        } catch (Exception e) {
    ...
    public boolean validateResetToken(String token) {
        System.out.println("🔍 Validating reset token: " + token.substring(0, 8) + "...");
        ...
        if (isValid) {
            ...
            System.out.println("❌ Token has expired. Expiry: " + resetToken.getExpiryDate() + ", Current: " + LocalDateTime.now());
        }
