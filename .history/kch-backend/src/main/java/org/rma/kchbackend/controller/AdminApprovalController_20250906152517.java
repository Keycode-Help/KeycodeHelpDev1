...
public class AdminApprovalController {
    ...
    /**
     * Grant trial approval to an admin account (Super Admin only)
     */
    @PostMapping("/trial-approve")
    public ResponseEntity<Map<String, Object>> trialApproveAdminAccount(@RequestBody Map<String, Object> request) {
        try {
            String superAdminEmail = (String) request.get("superAdminEmail");
            Long adminId = Long.valueOf((String) request.get("adminId"));
            String approvalNotes = (String) request.get("approvalNotes");
            ...
            boolean success = adminApprovalService.trialApproveAdminAccount(adminId, approvalNotes, superAdminEmail);
            
            if (success) {
                return ResponseEntity.ok(Map.of("message", "Trial approval granted successfully - 3 days access"));
            } else {
                return ResponseEntity.badRequest().body(Map.of("error", "Failed to grant trial approval"));
            }
        } catch (Exception e) {
            return ResponseEntity.status(500).body(Map.of("error", "Internal server error: " + e.getMessage()));
        }
