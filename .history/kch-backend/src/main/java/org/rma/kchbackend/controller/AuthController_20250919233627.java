...
public class AuthController {
    ...
    // Temporary endpoint to reset super admin password for debugging
    @PostMapping("/debug/reset-super-admin")
    public ResponseEntity<?> resetSuperAdminPassword(@RequestBody Map<String, String> request) {
        String email = request.get("email");
        String newPassword = request.get("password");
        
        System.out.println("=== DEBUG: Resetting super admin password ===");
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> loginRequest) {
        ...
        keycodeUserService.saveUser(user);
        System.out.println("✅ Last login time updated for user: " + user.getEmail());
    ...
    public ResponseEntity<Map<String, Object>> quickHealthCheck() {
        try {
            System.out.println("🔍 /auth/health/quick endpoint called - " + System.currentTimeMillis());
            
            Map<String, Object> healthInfo = new HashMap<>();
            healthInfo.put("status", "healthy");
            healthInfo.put("timestamp", System.currentTimeMillis());
            healthInfo.put("message", "Backend is responding");

            System.out.println("✅ Quick health check successful");
            return ResponseEntity.ok(healthInfo);
        } catch (Exception e) {
            System.out.println("❌ Quick health check failed: " + e.getMessage());
    ...
    public ResponseEntity<?> confirmPasswordReset(@RequestBody Map<String, String> request) {
        try {
            System.out.println("🔧 Password reset confirmation request received");
            
            String token = request.get("token");
            String newPassword = request.get("newPassword");
            
            System.out.println("🔑 Token: " + (token != null ? token.substring(0, 8) + "..." : "null"));
            ...
            if (newPassword == null || newPassword.trim().isEmpty()) {
                System.out.println("❌ New password is missing from request");
            ...
            // Validate token
            System.out.println("🔍 Validating reset token...");
    ...
    public ResponseEntity<?> changePassword(@RequestBody Map<String, String> request) {
        try {
            ...
            if (newPassword.matches(".*(.)\\1{2,}.*")) {
                return ResponseEntity.badRequest().body(Map.of("error", "Password cannot contain more than 2 consecutive identical characters"));
            }
            
            // Check for common sequences
            if (newPassword.matches(".*(123|abc|qwe|asd|zxc).*")) {
                return ResponseEntity.badRequest().body(Map.of("error", "Password cannot contain common sequences like 123, abc, etc."));
            }
            
            // Check for common dictionary words
            String[] commonWords = {"password", "123456", "admin", "user", "login", "welcome", "qwerty", "letmein"};
            String lowerPassword = newPassword.toLowerCase();
            for (String word : commonWords) {
                if (lowerPassword.contains(word)) {
                    return ResponseEntity.badRequest().body(Map.of("error", "Password cannot contain common words like '" + word + "'"));
                }
            }
