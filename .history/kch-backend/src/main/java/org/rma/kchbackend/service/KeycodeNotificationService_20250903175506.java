...
public class KeycodeNotificationService {
    ...
    public void sendKeycodeRequestNotifications(Vehicle vehicle, KeycodeUser user) {
        try {
            ...
            // Send email notification to customer (if preferences allow)
            if (notificationPreferencesService.shouldSendEmailNotification(user, "request")) {
                sendCustomerEmailNotification(customerEmail, customerName, vehicleInfo, "submitted");
            }

            // Send SMS notification to customer (if phone number available and preferences allow)
            if (customerPhone != null && !customerPhone.trim().isEmpty() && 
                notificationPreferencesService.shouldSendSmsNotification(user, "request")) {
    ...
    public void sendKeycodeStatusUpdateNotifications(Vehicle vehicle, String status) {
        try {
            ...
            if (notificationPreferencesService.shouldSendEmailNotification(user, "status")) {
                sendCustomerEmailNotification(customerEmail, customerName, vehicleInfo, status);
            }

            // Send SMS notification (if phone number available and preferences allow)
            if (customerPhone != null && !customerPhone.trim().isEmpty() && 
                notificationPreferencesService.shouldSendSmsNotification(user, "status")) {
    ...
    public void sendKeycodeCompletionNotifications(Vehicle vehicle) {
        try {
            ...
            // Send completion email (if preferences allow)
            if (notificationPreferencesService.shouldSendEmailNotification(user, "completion")) {
                sendCustomerCompletionEmail(customerEmail, customerName, vehicleInfo, hashedVin);
            }

            // Send completion SMS (if phone number available and preferences allow)
            if (customerPhone != null && !customerPhone.trim().isEmpty() && 
                notificationPreferencesService.shouldSendSmsNotification(user, "completion")) {
