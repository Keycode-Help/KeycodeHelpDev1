...
const UploadFileForm = ({ fileRef, name, onChange, accept, value, reset }) => {
    ...
    >
      ...
      {!value ? (
        ...
        <div className="flex py-1 md:py-2">
          ...
          <div
            className="p-2 rounded-full hover:bg-[#0f0f0f] border border-[#1A1A1A] transition-colors duration-200"
            onClick={(e) => {
              e.stopPropagation();
              reset();
            }}
          >
            <X className="size-7 text-red-600 hover:text-red-500" />
          </div>
        </div>
      )}
    </div>
